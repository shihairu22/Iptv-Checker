...
- 🔄 **数据合并更新**：针对相同地址的频道仅更新可访问状态和质量字段，保留用户自定义的名称、分组与 Logo。
- 🧠 **频道排序**：前端可自动按照画面质量规则进行排序操作，让高质量频道优先显示。
- 📤 **基础列表存储**：后台会自动更新并覆盖主检测结果数据存储。
- 🖼️ **可视化编辑**：提供频道信息的前端面板，并支持通过调用 PotPlayer 等播放器直接测试流。
...
- ⚙️ **环境隔离**:
  - 设置独立的检测参数和请求超时以适应不同网络环境。

---

## 📁 数据存储说明

所有的应用配置、用户手动修改以及扫描状态的数据，都仅依赖本地 `/data` 数据目录：
...
- 补齐 `logs.html` 所依赖的日志接口，支持文件列表、下载与实时流式查看
- 修复任务队列重复预取导致的重复探测问题，增加入队预占与恢复机制
- 批量检测写回时保留用户维护的频道元数据，避免覆盖分组、Logo、TVG 等字段
- 完善分离配置的加载、保存与备份恢复链路，补齐 app/proxy 等设置持久化
- 增加 Socket.IO 鉴权校验，并对播放器导出、台标、回看接口做兼容性修复

### v1.4.0 (2026-02-20)

- **前端架构彻底重构**: 引入 `app.js` 与 `utils.js` 拆分庞大的内联脚本。
- **并发引擎优化**: 抛弃负担极大的进程池，改用轻量级异步 `p-queue` 架构，支持单机并发飙升至 200+。
- **WebSocket 实时通信**: 引入 `socket.io` 底层通道替换轮询机制，实现平滑高效的数据加载和统计呈现。
- **智能预检技术**: 搭载基于底层 Socket 的 100ms 纯死链过滤预判机制。
- **快照持久化恢复**: 找回并重写完整的基于 JSON 文件覆盖的回滚机制及相关界面入口。
